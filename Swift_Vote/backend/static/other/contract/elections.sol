...
contract Election {
    ...
    function init(address _ad) private{//adding the init user 
        userCount+=1;
        userAddress[_ad]=userCount;
        users[userAddress[_ad]]=user(userCount,0,1);
    }
    
    function addUser(address _ad) public{//function to add users be it candidates or voters or officials.
    ...
    function userDetails(address _ad) public view returns (user memory){//a tester function and for manual verification.
        require(users[userAddress[msg.sender]].ablity_to_add==1,"the role doesnt allow it");
        return users[userAddress[_ad]];
    }

    function listCandidates() public view returns (string[] memory,string[] memory){// a viewer function to get the list of candidates to conduct voting
    ...
    function DisableAblity(address _ad) private { //function to not allow a person to vote more than once.
    ...
    function cvote(uint256 _cid,string memory _location,string memory _date) public {//cvote for cast vote
